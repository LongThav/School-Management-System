...
import '../student/create_account_student.dart';
import '../student/update_student_infor_view.dart';
...
class _TeacherViewState extends State<TeacherView> {
  ...
  Widget _listStudent(List<User> user, double width, double height) {
    ...
    for (var student in user) {
      if (student.role?.role == "Teacher" || student.role?.role.toLowerCase() == "teacher" || student.role?.role == "Parent" || student.role?.role == "parent") {
        ...
        rows: List.generate(students.length, (index) {
          return DataRow(
              onSelectChanged: (bool? value) {
                setState(() {
                  _teacherSex = students[index].gender?.sex ?? "";
                  _teacherRole = students[index].role?.role ?? "";
                });
                context.read<TeacherBloc>().add(TeacherEvent(students[index].id));
  ...
  Widget _buildProfileStudent() {
    double height = MediaQuery.of(context).size.height;
    return BlocBuilder<TeacherBloc, ApplicationState>(builder: (context, state) {
      ...
      } else if (state is TeacherByIDState) {
                      ...
                      onTap: () async {
                        if (data.data.userInfor?.codeId == null) {
                          ...
                          Future.delayed(const Duration(milliseconds: 600), () {
                            context.read<UserBloc>().add(UserEvent());
                            context.read<TeacherBloc>().add(TeacherEvent(data.data.id));
                          });
                        } else {
